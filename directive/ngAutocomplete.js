...
//              if (scope.details) {
                            ...
                            scope.details.address_components.forEach(function (item) {

                                for(var i= 0;i <= item.types.length;i++){
                                    if(item.types[i] == 'street_number'){
                                        console.log(item.short_name);
                                        locationAddress.street_number = item.short_name;
                                    }
                                    else if(item.types[i] == 'political'){
                                        console.log(item.short_name);
                                        locationAddress.city = item.short_name;
                                    }else if(item.types[i] == 'political'){
                                        console.log(item.short_name)
                                        locationAddress.state = item.short_name;
                                    }else if(item.types[i] == 'country'){
                                        console.log(item.short_name)
                                        locationAddress.countery = item.short_name;
                                    }
                                }
